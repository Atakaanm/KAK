...
1. ✅ **Unity modülleri** (2026-09-26): iOS Build Support + Android Build Support (OpenJDK, SDK 34/35/36, NDK, CMake) kuruldu. Yer açmak için kullanılmayan WebGL modülü kaldırıldı.
2. ✅ **Xcode 26.6** kurulu ve seçili (iOS 26.5 SDK + simülatör). Oyun iPhone 17 Pro simülatöründe çalıştı (`tools/kak_ios_sim.sh`).
...
7. **Build:** Claude'a "Android build al" / "iOS projesi çıkar" de (`KacAtaKac/Yayın/Android Release (.aab)` ve `iOS Xcode Projesi`). iOS'u **kendi iPhone'unda** denemek için ücretli hesap gerekmez: Xcode → Settings → Accounts'a Apple kimliğinle gir (ücretsiz "Personal Team", uygulama 7 gün çalışır), iPhone'u kabloyla bağla, Claude projeyi açar ve cihaza kurar.
