...
**Bağlılık (Faz 3c):** `FeatureGate` (adım adım açılma; `Feature` enum + `IsUnlocked`), altın: `Coin` (havuzlu) + `CoinSpawner` (Managers) + `CoinHud` (HudBand) + `ScoreManager.Coins/AddCoins` + `GameManager.RunCoins/RunCoinBonus` → `SaveData.coins/totalCoins`, `GameEvents.CoinCollected/CoinClusterSpawned`. Kurulum: `KacAtaKac/Bağlılık Katmanını Kur (Faz 3c)` (`KakMetaSetup`). Tasarım: `3c.md`.
