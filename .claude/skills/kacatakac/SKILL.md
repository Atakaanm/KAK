...
- **[yayin.md](yayin.md):** Mağaza yayını ve reklam hazırlığı (Y1-Y5): gereksinimler (`Docs/Yayin-Gereksinimleri.md`), teknik ayarlar, reklam altyapısı (`Docs/Reklam-Hazirlik.md`).
...
**Bağlılık (Faz 3c):** `FeatureGate` (adım adım açılma; `Feature` enum + `IsUnlocked/IsNew/LockedHint/UnlockedMask`; menü `FeatureButton`, oyun sonu `GameManager.NewlyUnlocked` afişi, menü `WalletHud`), altın: `Coin` (havuzlu) + `CoinSpawner` (Managers) + `CoinHud` (HudBand) + `ScoreManager.Coins/AddCoins` + `GameManager.RunCoins/RunCoinBonus` → `SaveData.coins/totalCoins`, `GameEvents.CoinCollected/CoinClusterSpawned`. Karakterler: `CharacterCatalog` (Resources) + `PlayerData` istatistikleri → `LevelManager.CurrentCharacter` / `ApplyPlayerData` (can, hız, dash, gövde, kalkan, powerup sıklığı/süresi, altın çarpanı); menü `CharacterPanel` + `CharacterCard`. Cila: `UiConfetti` (yeni rekor), `NextGoal` (sıradaki satın alma hedefi; oyun sonu çubuğu, menü `FeatureButton.buyDot`). Günlük ödül: `DailyReward` (Core) + menü `DailyRewardPanel` (MainMenuController açılışta açar). Petler: `PetCatalog` (Resources) + `PetData` → `PetFollower.Spawn` (LevelManager; FeatureGate.Pets), menü `PetPanel`. Görevler: `MissionSystem` (Core; `SaveData.missions/missionsCompleted`, `RunStats`), oyun sonunda `GameManager` işler, `GameOverScreen.ShowMissions`. Kurulum: `KacAtaKac/Bağlılık Katmanını Kur (Faz 3c)` + `Karakterleri Kur (Faz 3c)` (`KakMetaSetup`). Tasarım: `3c.md`.
**Reklam (Y3, varsayılan kapalı):** `AdService` (tek giriş: `CanShow`/`ShowRewarded`), `AdConfig` (Resources), sağlayıcılar Null/Simulated/AdMob(`KAK_ADMOB`). Ölüm → `GameManager`: teklif varsa `ContinuePanel`, kayıt `FinalizeRun` ile bir kez; `Revive`. Oyun sonu 2× altın. **Yayın:** `KakBuild` (API 36, AAB, iPhone-only), `KakPlatformPostBuild` (Android appCategory, iOS plist), `StoreLinks.PrivacyUrl`.
...
| Dev/* | `KakDevMenu` | bot, ölümsüzlük, kalkan ver, tüm powerup'lar, `WalkPlayer`, `SetGamesPlayed n`, `SetCoins n`, `PlayAs <id>`, `PlayWithPet <id>`, `PressCharacterCard i`, `PressPetCard i`, `OpenPets`, `OpenDaily <seri>`, `ClaimDaily`, `SetBest n`, dünya yazısı, UI panelleri |
